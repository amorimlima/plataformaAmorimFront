...
$(document).ready(function(){
    ...
    $("[id^='Grupo_Id_'] .dataPresenca").change(function() {
        var grupo = $(this).parent()[0].id.split("_")[2];
        var diaChamada = $(this);

        for (var i = 0; i < $('.Aluno_Grupo' + grupo).length - 1; i++)
        {
            var alunoId = $('.Aluno_Grupo' + grupo)[i].id.split("_")[1];
            var presenca = -1;
            var chamadaId;

            $.ajax({
                url: path + "Chamada/dataChamada/" + alunoId + "/" + diaChamada.val().split("/")[2] + "-" + diaChamada.val().split("/")[1] + "-" + diaChamada.val().split("/")[0],
                type: "GET",
                async: false,
                crossDomain: true,
                success: function(data){
                    ...
                    {
                        chamadaId = data[0].idchamada
                        existe = data[0].presenca;
                    }
             ...
             if (existe >= 0)
            {
                $("#Aluno_" + alunoId + " [class^='status_presenca']")[0].id = "chamada_" + chamadaId;
                if (existe == 1)
                {
                    $("#Aluno_" + alunoId + " [class^='status_presenca']").addClass("status_presenca_presenca");
                    $("#Aluno_" + alunoId + " [class^='status_presenca']").removeClass("status_presenca_falta");
                }
                else
                {
                    $("#Aluno_" + alunoId + " [class^='status_presenca']").addClass("status_presenca_falta");
                    $("#Aluno_" + alunoId + " [class^='status_presenca']").removeClass("status_presenca_presenca");
                }
            ...
            {
                $("#Aluno_" + alunoId + " [class^='status_presenca']")[0].id = '';
                $("#Aluno_" + alunoId + " [class^='status_presenca']").addClass("status_presenca_presenca");
                $("#Aluno_" + alunoId + " [class^='status_presenca']").removeClass("status_presenca_falta");
            }
        }
