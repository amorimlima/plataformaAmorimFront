...
function getUltimoPlanoEstudo(){
    ...
    $.ajax({
        ...
        crossDomain: true,
        success: function(d) {
            var diaFimUTC = Date.UTC(d[0].dataFim.split("-")[0], d[0].dataFim.split("-")[1], d[0].dataFim.split("-")[2]);
            var dataAtual = new Date();
            var diaAtualUTC = Date.UTC(dataAtual.getFullYear(), dataAtual.getMonth()+1, dataAtual.getDate())

            if (diaFimUTC < diaAtualUTC)
            {
                retorno = 0;
            }
            else
    ...
    if(tamanhoArquivo == "menor"){
        if(Arquivo != "" && Arquivo != undefined) { 
            $.ajax({
                ...
                type: "POST",
                crossDomain: true,  
                data: "action=create&anoLetivo="+anoLetivoId()+"&texto="+$('#Roteiro_Id_'+roteiroAcionado+' .roteiro_nome_tabela_texto').html()+"&data="+dataSalvaPortifolio.getUTCFullYear()+"-"+(dataSalvaPortifolio.getUTCMonth()+1)+"-"+dataSalvaPortifolio.getUTCDate()+"&aluno="+getAlunoByUsuario(usuarioId)+"&tipo="+tipoProducao+"&categoria=1&roteiro="+roteiroAcionado+"&status=&capa=",    
                beforeSend: function(){
                    loading("inicial");
                }, 
                success: function(d) {
...
function showUpload(Numero, ID)
{
    ...
    var HtmlContentUpload = '<div id="JanelaUploadPortifolio">'+
                                ...
                                '<form id="Cadastro_Producao_Aluno">'+
                                    ...
                                    '<div class="campoConfirmaUpload">'+
                                        '<input class="btn_submit" onclick="SalvarPortifolio('+tipoProdAnluno+', '+ID+')" type="button" value="" />'+
                                    '</div>'+
                                '</form>'+
                            '</div>';
